...
`workspace/external-memory/memory/` is portable advisory memory for improving the AI Workflow system itself.
...
The router/index is `workspace/external-memory/external-memory.md`.
...
- `workspace/repo/core/memory.md` and `workspace/repo/memory/` as repo-specific memory router and entries;
- `workspace/projects/<project>/memory.md` and `workspace/projects/<project>/memory/` as project-specific memory router and entries;
...
| --- | --- | --- | --- |
| `workspace/external-memory/memory/` | universal workflow/process memory | cross-repo workflow improvements, reusable autopilot rules, process anti-patterns, template and skill improvements | repo facts, domain facts, task-specific implementation knowledge |
...
| `workspace/repo/core/memory.md` and `workspace/repo/memory/` | one repository | durable facts about this repo, local constraints, local history | generic workflow improvements meant for other repos |
| `workspace/projects/<project>/memory.md` and `workspace/projects/<project>/memory/` | one project/workspace | durable project decisions, project architecture facts, project-specific lessons | generic workflow system design |
| `workspace/projects/<project>/distillations/` | one completed task/package | task-level implementation lessons after PASS | broad rules unless promoted by checkpoint |
...
After adding or updating an entry, update `workspace/external-memory/external-memory.md` with only date, topic, type, status, and route.
...
- local target skill -> `workspace/skills/`;
- autopilot rule -> `.systems/ai/core/autopilot.md`;
- repo bootstrap requirement -> `workspace/repo/core/repo-intake.md`.
