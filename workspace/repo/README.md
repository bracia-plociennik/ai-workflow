...
`workspace/repo/` is the only canonical home for global facts about the repository that installed this workflow template. In the default nested-clone install, this directory lives under `AI_WORKFLOW_HOME`, usually `ai-workflow/workspace/repo/` from the target repo root.
...
- Do not store project task specs, QA evidence, or autopilot runtime here; those belong in `workspace/projects/<project>/`.
- Do not store universal workflow lessons here; those belong in `workspace/external-memory/memory/` and are indexed by `workspace/external-memory/external-memory.md`.
