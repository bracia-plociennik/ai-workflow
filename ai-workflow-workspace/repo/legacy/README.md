...
The router/index is `ai-workflow-workspace/repo/core/legacy.md`.
...
If a legacy file may contain sensitive content, record only its original path and `owner review required` in `ai-workflow-workspace/repo/core/repo-intake.md`.

## Naming

Preserved legacy files are exempt from `.systems/scripts/check-naming`.
