...
`ai-workflow-workspace/external-memory/memory/` is portable advisory memory for improving the AI Workflow system itself.
...
The router/index is `ai-workflow-workspace/external-memory/external-memory.md`.
...
- `.systems/ai/core/workflow.md` and `.systems/ai/workflow/` as workflow rules;
- `.systems/ai/core/memory.md` and `.systems/ai/memory/` as template-local memory router and entries;
- `ai-workflow-workspace/repo/core/memory.md` and `ai-workflow-workspace/repo/memory/` as repo-specific memory router and entries;
- `ai-workflow-workspace/projects/<project>/memory.md` and `ai-workflow-workspace/projects/<project>/memory/` as project-specific memory router and entries;
...
| --- | --- | --- | --- |
| `ai-workflow-workspace/external-memory/memory/` | universal workflow/process memory | cross-repo workflow improvements, reusable autopilot rules, process anti-patterns, template and skill improvements | repo facts, domain facts, task-specific implementation knowledge |
| `.systems/ai/core/memory.md` and `.systems/ai/memory/` | this workflow template | template-local maintenance facts and history | target-repo facts or universal workflow lessons |
| `ai-workflow-workspace/repo/core/memory.md` and `ai-workflow-workspace/repo/memory/` | one repository | durable facts about this repo, local constraints, local history | generic workflow improvements meant for other repos |
| `ai-workflow-workspace/projects/<project>/memory.md` and `ai-workflow-workspace/projects/<project>/memory/` | one project/workspace | durable project decisions, project architecture facts, project-specific lessons | generic workflow system design |
| `ai-workflow-workspace/projects/<project>/distillations/` | one completed task/package | task-level implementation lessons after PASS | broad rules unless promoted by checkpoint |
...
In target repositories, do not apply those updates directly to `.systems/**`. Promote them through the official upstream `ai-workflow` repository.
...
Use `.systems/ai/templates/external-memory/date-external-memory.template.md` for new entries.

After adding or updating an entry, update `ai-workflow-workspace/external-memory/external-memory.md` with only date, topic, type, status, and route.
...
- universal process rule -> `.systems/ai/workflow/README.md`;
- agent execution rule -> `AGENTS.md`;
- human operating guidance -> `HUMANS.md`;
- template change -> `.systems/ai/templates/`;
- system skill improvement -> `.systems/ai/skills/` in the official upstream repository;
- local target skill -> `ai-workflow-workspace/skills/`;
- autopilot rule -> `.systems/ai/core/autopilot.md`;
- repo bootstrap requirement -> `ai-workflow-workspace/repo/core/repo-intake.md`.
