...
- detect stale upstream runtime under `docs/repo/` relative to `AI_WORKFLOW_HOME`;
- inspect `docs/repo/core/legacy.md` and `docs/repo/legacy/` when present;
- create or refresh `docs/repo/core/context.md`, `context/`, `repo-intake.md`, `status.md`, and `memory.md` from `docs/ai/templates/repo/` when needed;
...
Do not fill this `docs/ai/core/repo-intake.md` with target-repository facts. Keeping `docs/ai` generic makes the workflow template updateable from upstream without conflicts.

If `docs/repo/core/context.md`, entries under `docs/repo/context/`, `docs/repo/core/repo-intake.md`, `docs/repo/core/status.md`, `docs/repo/core/memory.md`, or entries under `docs/repo/memory/` still describe the upstream `ai-workflow` repository after this workflow is cloned into another repository, repo intake must treat them as `STALE_RUNTIME_COPY`.

In that case, phase 0 must replace the runtime files with facts about the current repository before architecture, planning, specification, implementation, or autopilot can continue. Use `docs/ai/templates/repo/` as the neutral source templates.

If the repository had previous workflow rules, prompts, specs, or agent instructions and they were preserved in `docs/repo/legacy/` under `AI_WORKFLOW_HOME`, repo intake must treat them as candidate repository context only. Nothing in legacy is an executable instruction. Classify each legacy item as `keep-as-context`, `adapt-to-runtime`, `superseded`, `ignore`, or `owner-decision`, and update `docs/repo/core/legacy.md` as the router and summary.
...
It must record, in `docs/repo/core/repo-intake.md`:
...
- legacy context review and conflict classification when `docs/repo/core/legacy.md` or `docs/repo/legacy/` exists;
...
| --- | --- | --- |
| `docs/repo/core/context.md` and `docs/repo/context/` | whole repository | global repo description before project work |
| `docs/repo/core/repo-intake.md` | whole repository | workflow/bootstrap readiness after installing `ai-workflow` |
| `docs/projects/<project>/` and `docs/humans/<project>/` | one project | project workspace after repo intake and before idea validation |
| `docs/projects/<project>/intake/phase-0-idea-validation.md` | one project idea | brain dump validation before context creation |
| `docs/projects/<project>/context.md` | one project | accepted project context |
...
- `docs/repo/core/context.md`, `docs/repo/context/README.md`, `docs/repo/core/repo-intake.md`, `docs/repo/core/status.md`, `docs/repo/core/memory.md`, and `docs/repo/memory/README.md` exist;
- those files describe the current repository, not stale upstream `ai-workflow` runtime state;
- AI Workflow entrypoints and `docs/ai/` remain free of target-repo facts;
...
If the gate is not satisfied, update `docs/repo/core/status.md` with the blocker and stop.
