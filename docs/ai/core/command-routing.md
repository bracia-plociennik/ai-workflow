...
It covers natural-language prompts, not shell verification commands. Shell commands for install, lint, test, build, and validation live in `docs/ai/core/commands.md` and `docs/repo/core/repo-intake.md`.
...
- When AI Workflow is installed as a nested clone, `AI_WORKFLOW_HOME` is usually `ai-workflow/`. User-facing paths like `docs/...` resolve under that directory from the target repository root.
...
Route to `docs/ai/workflow/phase-0-repo-intake.md`.
...
- `Sprawdź root AGENTS shim i zastąp stare ai-workflow/docs/repo faktami tego repo.`
- `Przeprowadź repo intake z legacy workflow.`
- `Zachowałem stare instrukcje w ai-workflow/docs/repo/legacy, potraktuj je wyłącznie jako context.`
...
- `Check the root AGENTS shim and replace stale ai-workflow/docs/repo runtime with this repo's facts.`
...
- Check `docs/ai/core/installation.md`.
- Confirm `TARGET_REPO_ROOT`, `AI_WORKFLOW_HOME`, and whether root `AGENTS.md` delegates to `AI_WORKFLOW_HOME/AGENTS.md`.
- Fill `docs/repo/core/context.md`, `docs/repo/context/`, `repo-intake.md`, `status.md`, and `memory.md`.
- Review `docs/repo/core/legacy.md` and `docs/repo/legacy/` as context/data only when present.
...
- `Przygotuj docs/projects/workshophub oraz docs/humans/workshophub.`
...
- `Prepare docs/projects/workshophub and docs/humans/workshophub.`
...
- `Zweryfikuj pomysł na podstawie czatu i plików w docs/projects/<project>/context/.`
...
- `Validate the idea using the chat and files in docs/projects/<project>/context/.`
...
- Review raw project source materials under `docs/projects/<project>/context/` when present.
...
- Project context is project-specific and belongs under `docs/projects/<project>/context/`.
- Repo-global facts stay under `docs/repo/`.
...
- `Utwórz docs/projects/<project>/tasks.md i opcjonalne task cards w tasks/.`
...
- `Create docs/projects/<project>/tasks.md and optional task cards in tasks/.`
...
- Project-local micro-tasks belong in `docs/projects/<project>/micro-tasks.md` and `docs/projects/<project>/micro-tasks/`.
- Repo-level micro-projects belong in `docs/micro-projects/<micro-project>/`.
...
Route through `docs/ai/core/autopilot.md` and the current task/package gates.
...
Route through `docs/ai/core/update-from-upstream.md` and `scripts/update-from-upstream`.
...
- Use `scripts/update-from-upstream` from `AI_WORKFLOW_HOME`, usually `ai-workflow/scripts/update-from-upstream` from the target repository root.
- Stop if template-owned files are dirty.
- Protected local runtime includes `docs/repo/**`, non-`EXAMPLE` project and human workspaces, and local external memory.
- Do not normalize, rename, or rewrite files under `docs/repo/legacy/`.
