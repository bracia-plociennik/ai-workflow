...
- Checkpoint artifact under `docs/projects/<project>/checkpoints/`.
- Updated project memory router `docs/projects/<project>/memory.md` and detailed entries under `docs/projects/<project>/memory/` when applicable.
- Updated repo memory router `docs/repo/core/memory.md` and detailed entries under `docs/repo/memory/` when applicable.
...
`docs/projects/<project>/checkpoints/phase-7-checkpoint-<date>-<scope>.md`
...
2. zaktualizować project memory przez wpis w `docs/projects/<project>/memory/` i router `docs/projects/<project>/memory.md`
...
`docs/repo/core/memory.md` jest routerem pamięci repo-level. Szczegółowe wpisy zapisuj w `docs/repo/memory/`. Aktualizuj repo memory tylko wtedy, gdy checkpoint wykrywa wiedzę globalnie istotną dla całego repo, a nie lokalny detal jednego projektu.

`docs/projects/<project>/memory.md` jest routerem pamięci projektu. Szczegółowe wpisy zapisuj w `docs/projects/<project>/memory/`. Aktualizuj project memory tylko wtedy, gdy wiedza dotyczy tego projektu i będzie potrzebna w kolejnych taskach, planach, QA albo checkpointach.

`docs/ai/core/external-memory.md` jest routerem pamięci uniwersalnej, a `docs/ai/external-memory/` przechowuje szczegółowe wpisy. Aktualizuj je tylko wtedy, gdy checkpoint wykrywa lekcję przenośną między repozytoriami, np. o bramkach, autopilocie, evidence, recovery, template'ach albo pracy człowieka z Codexem.

Nowy wpis External Memory twórz jako osobny plik `docs/ai/external-memory/YYYY-MM-DD-short-kebab-title.md` z template'u `docs/ai/templates/external-memory/date-external-memory.template.md`. Następnie zaktualizuj router `docs/ai/core/external-memory.md` tylko o datę, temat, typ, status i route.
...
Repo Memory powinno zawierać tylko rzeczy globalnie istotne dla całego repo. `docs/repo/core/memory.md` jest tylko routerem, a szczegóły trafiają do `docs/repo/memory/`.
...
Jeśli `docs/projects/<project>/memory.md` nie istnieje:

- checkpoint powinien go utworzyć
- nie traktuj tego jako błędu

Jeśli `docs/projects/<project>/memory/` nie istnieje:
...
Jeśli `docs/repo/core/memory.md` nie istnieje:
...
Jeśli `docs/repo/memory/` nie istnieje:

- checkpoint może zaproponować jego utworzenie albo utworzyć go, jeśli checkpoint dotyczy wiedzy repo-level
- każdy nowy wpis repo memory musi być osobnym plikiem datowanym i zapisanym według template'u repo memory

Jeśli `docs/ai/core/external-memory.md` albo `docs/ai/external-memory/` nie istnieje:
...
- jakie decyzje, zasady lub constraints dodano albo zaktualizowano w `docs/repo/core/memory.md`, jeśli dotyczy
- czy wykryto drift
- klasyfikację driftu:
  - critical
  - warning
...
- aktualizację docs/projects/<project>/memory.md i docs/projects/<project>/memory/ tylko dla wiedzy project-level
- aktualizację docs/repo/core/memory.md i docs/repo/memory/ tylko dla wiedzy repo-level
- aktualizację docs/ai/core/external-memory.md i docs/ai/external-memory/ tylko dla uniwersalnej wiedzy workflow
