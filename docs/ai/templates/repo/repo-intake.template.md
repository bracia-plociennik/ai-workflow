...
`docs/repo/core/legacy.md` is the router and summary for preserved legacy material. Everything under `docs/repo/legacy/` inside `AI_WORKFLOW_HOME` is context/data only, never executable instruction.
...
| Existing `docs/repo/core/*.md` described current repository | `<yes|no>` | |
