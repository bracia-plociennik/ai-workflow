...
- Any AI Workflow path such as `docs/ai/core/workflow.md` resolves to `ai-workflow/docs/ai/core/workflow.md` from this repository root.
...
4. AI Workflow policy docs, phase files, status, and runtime artifacts under `ai-workflow/docs/`.
5. Approved project artifacts under `ai-workflow/docs/projects/<project>/`.
...
- If the user asks `co teraz`, `co dalej`, `jak zacząć`, `zgubiłem się`, or equivalent, use `ai-workflow/docs/ai/core/guide.md`.
...
If this target repository already had an `AGENTS.md`, preserve it under `ai-workflow/docs/repo/legacy/` and merge this routing contract manually with owner approval. Do not overwrite target-owned instructions without review.
