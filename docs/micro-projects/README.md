...
Micro-projects are small, self-contained work items that do not need a full `docs/projects/<project>/` workspace, architecture, project plan, spec QA, quality phase, distillation, checkpoint, or autopilot run.
...
- Real target-repository micro-projects are target-owned runtime and are protected by `scripts/update-from-upstream`.
