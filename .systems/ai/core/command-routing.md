...
- Full QA Verification from `full-qa-verification.md` applies an artifact-appropriate review of intent, DoD, scope, evidence, findings/blockers, and relevant artifact/diff. For data, integration, state-flow, parser, or executable-entrypoint changes, it also requires the adaptive data/integration matrix.
