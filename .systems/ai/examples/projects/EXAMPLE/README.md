# EXAMPLE Project Workspace

This is a complete example project workspace showing every supported project artifact type.
It is not active workflow state and must not be used as the source of truth for real implementation.

## Included Artifact Types

- Root support files: `status.md`, `memory.md`, `plans.md`, `tasks.md`, `micro-tasks.md`, `change-requests.md`, `code-review.md`
- Project memory entries: `memory/`
- Task cards: `tasks/`
- Micro-task router and entries: `micro-tasks.md`, `micro-tasks/2026-05-20-example-micro-task.md`
- Change request router and entries: `change-requests.md`, `change-requests/`
- Project context: `context.md` plus supporting materials in `context/`
- Phase 0: `intake/phase-0-idea-validation.md`, `intake/phase-0-repo-intake.md`
- Phase 1: `architecture/phase-1-architecture.md`
- Phase 1.5/1.7: `quality/phase-1-architecture-qa.md`, `quality/phase-1-architecture-fix-loop.md`
- Phase 2: `planning/phase-2-project-plan.md`
- Phase 2.5/2.6/2.7/2.9/2.9.1: plan QA, fix loop, packaging, packaging QA, package fix loop quality artifacts
- Phase 3: `specs/phase-3-ex-01-example-task-specification.md`
- Phase 3.5/3.7: spec QA and spec fix loop quality artifacts
- Phase 4/5/5.5: implementation result, quality, and fix loop artifacts
- Phase 6: `distillations/phase-6-ex-01-example-task-distillation.md`
- Phase 7: `checkpoints/phase-7-checkpoint-2026-05-16-example.md`
- Phase 8: `quality/phase-8-final-check.md`
- Decisions: `decisions/ex-01-example-task-decisions.md`
- Escalations: `escalations/example-escalation-2026-05-16.md`
- Reviews: `reviews/2026-05-16-ex-docs-001-example-task-review.md`
- Autopilot router and runtime examples: `autopilot/README.md`, `autopilot/runs/autopilot-001/state.md`, `autopilot/runs/autopilot-001/ledger.md`, `autopilot/runs/autopilot-001/events.md`

## Warning

This workspace is documentation-only. Do not mark real work PASS based on EXAMPLE artifacts.
